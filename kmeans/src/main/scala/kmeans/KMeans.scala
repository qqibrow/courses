...
class KMeans {
  ...
  def classify(points: GenSeq[Point], means: GenSeq[Point]): GenMap[Point, GenSeq[Point]] = {
    val s = points.groupBy(findClosest(_, means))
    val notAddedOnes = means.filter(!s.contains(_))
    s ++ notAddedOnes.map(p => (p, GenSeq[Point]()))
  }
  ...
  def update(classified: GenMap[Point, GenSeq[Point]], oldMeans: GenSeq[Point]): GenSeq[Point] = {
    oldMeans.map { p =>
      val points = classified(p)
      findAverage(p, points)
    }
  }
